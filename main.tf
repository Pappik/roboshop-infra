...
module "alb" {
  ...
  vpc_id = lookup(lookup(module.vpc, each.value.vpc_name, null), "vpc_id", null)
  allow_cidr = each.value.internal ? concat(lookup(lookup(lookup(lookup(module.vpc, "main" , null),"private_subnet_ids", null), "web", null),"cidr_block", null), lookup(lookup(lookup(lookup(module.vpc, "main" , null),"private_subnet_ids", null), "app", null),"cidr_block", null)): ["0.0.0.0/0"]
